...
if (isset($_POST["submit"])) {
	...
	if (emptyInputSignup($CID, $first, $last, $email, $username, $pwd, $pwdRepeat) !== false) {
		header("Location: ../signupteacher.php?error=emptyinput");
		exit();
	}
	if (invalidUid($username) !== false ) {
		header("Location: ../signupteacher.php?error=invaliduid");
		exit();
	}
	if (invalidEmail($email) !== false ) {
		header("Location: ../signupteacher.php?error=invalidEmail");
		exit();
	}
	if (pwdMatch($pwd, $pwdRepeat) !== false ) {
		header("Location: ../signupteacher.php?error=passwordsdontmatch");
		exit();
	}
